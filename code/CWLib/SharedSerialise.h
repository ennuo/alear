...
#define ADD_SR(revision, name) if (r.GetRevision() >= revision && (rv = Add(r, d.name, #name)) != REFLECT_OK) return rv;
#define ADD(name) rv = Add(r, d.name, #name); if (rv != REFLECT_OK) return rv;

#define ADD_ARRAY_ELEMENT(name, index) rv = Add(r, d.name[index], #name "_" #index); if (rv != REFLECT_OK) return rv;
#define ADD_CONDITIONAL(revision, condition, name) if (condition) { ADD_SR(revision, name); }
