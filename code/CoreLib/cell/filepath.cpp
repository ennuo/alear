...
bool FileOpen(const CFilePath& fp, FileHandle& fd, EOpenMode mode) 
{
	...
	switch (mode) 
	{
		...
		case OPEN_SECURE:
		{
			if (StringCompare(fp.GetExtension(), ".sdat") == 0)
			{
				ret = cellFsSdataOpen(fp.c_str(), 0, &fd, NULL, 0);
				break;
			}

			static SceNpDrmKey key =
			{
				{
					0x74, 0x91, 0x9b, 0x05,
					0xa6, 0xa2, 0xb1, 0x80,
			...
			ret = sceNpDrmOpen(&key, fp.c_str(), 0, &fd, NULL, 0);
			break;
		}
...
{
	return cellFsUnlink(fp) == CELL_FS_OK;
}
