...
CJobManager::CJobManager(u32 num_threads, const char* thread_name) : NextUniqueTag(1), NextUniqueJobID(1),
Threads(), WorkQueue(), ActiveQueue() // 7
{
    for (int i = 0; i < num_threads; ++i)
        Threads.push_back(ThreadCreate(WorkerThreadFunctionStatic, (uint64_t)this, thread_name, 1000, 0x10000, true));
}
